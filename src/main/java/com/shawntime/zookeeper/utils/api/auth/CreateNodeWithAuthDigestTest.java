 ...
 */
public class CreateNodeWithAuthDigestTest implements Watcher {
    ...
    public static void main(String[] args) {
        try {
            zooKeeper = new ZooKeeper("127.0.0.1:2181", 5000, new CreateNodeWithAuthDigestTest());
            countDownLatch.await();
            createNodeWithAuth();
            getNodeWithDigestAuth();
            setNodeWithDigestAuth();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static void getNodeWithDigestAuth() {
        zooKeeper.addAuthInfo("digest", "password:123456".getBytes());
        Stat stat = new Stat();
        try {
     ...
     */
    private static void setNodeWithDigestAuth() {
        Stat stat = new Stat();
        try {
            zooKeeper.setData("/api_create", "".getBytes(), -1);
        } catch (KeeperException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
            e.printStackTrace();
